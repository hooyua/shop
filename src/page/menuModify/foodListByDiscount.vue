            ...
            <li class="order_list_li" v-for="(item,index) in footList" :key="item.id">
                <section class="order_item_right">
                    <section class="order_basket">
                        ...
                        <span
                            style="color:grey;font-size:.5rem;"
                            v-if="item.discount == '1'"
                            class="description_foodname"
                        >なし</span>
                        <span
                            style="color:grey;font-size:.5rem;"
                            v-else
                        ...
                        <img
                            v-if="item.checked"
                            @click="checkbox_click2(index)"
                            :src="GLOBAL.defaults.choose"
                            class="imgCheck2"
                        >
                        <img
                            v-else
                            @click="checkbox_click2(index)"
                            :src="GLOBAL.defaults.unchoose"
                            class="imgCheck2"
                        >
                    </section>
                </section>
            ...
            <div class="alet_containers">
                ...
                <section class="cover-contents">
                    ...
                    <div class="sa-bottons">
                        ...
                        <div class="confrims" @click="discount_fun">確認</div>
                    </div>
                </section>
            </div>
        </section>
        <!-- 打折滑动选择器 -->
        <section class="coverparts" v-if="pickerShow">
            <div class="alet_containers">
                <section class="cover-backgrounds"></section>
                <section class="cover-contents">
                    <div class="pickerW">
                        <picker :maps="maps" :map.sync="map"></picker>
                    </div>
                    <div class="pickerW">
                        <picker :maps="maps" :map.sync="map2"></picker>
                    </div>
                    <div class="pickerW">
                        <p>%OFF</p>
                    </div>
                    <div class="sa-bottons">
                        <div class="confrim" @click="waitingPicker">キャンセル</div>
                        <div class="confrims" @click="confrimPicker">確認</div>
                    </div>
                </section>
            </div>
        </section>
        <section class="confrim_order">
            <p v-if="pickerDiscount>=0" @click="discountPicker()">割引率:{{pickerDiscount}}%</p>
            <p v-else @click="discountPicker()">割引率設定</p>
            <p @click="discountConfirm()">確認</p>
        </section>
...
<script>
    ...
    import {getMenuDetail, updateDiscount} from 'src/service/getData'
    ...
    export default {
      data(){
            return{
                ...
                show: false,            //显示菜品详细内容
            pickerShow: false, //打折滑动选择器
            pickerDiscount: -1, //打折百分比
                qriouslyimgs: null,
                shopid: null,
                showAlert: false,
                alertText: null,
        ...
        methods: {
            ...
            async initData(){
                ...
                for (var i = 0;i < this.tablelist.foods.length; i++) {
                    ...
                    this.footList.push(
                    {
                        ...
                        discount:   this.tablelist.foods[i].discount,
                        checked:    checked
                    });
                }
                this.maps = [
            ...
            //打折确认
            discountConfirm(discount){
                if(this.pickerDiscount==-1){
                    this.showAlert = true;
                    this.alertText = "割引率設定しない";
                    return;
                }
                let count = 0;
                for (var i = 0; i < this.footList.length; i++) {
                    if (this.footList[i].checked) {
                    count++;
                    }
                }
                if (count == 0) {
                    this.showAlert = true;
                    this.alertText = "料理を選択してください";
                    return;
                } else {
                        this.putonShow = true;
            ...
            async discount_fun(){
                ...
                for (var i = 0;i < menu_puton.length; i++) {
                    var data = {
                        ...
                        sold_out:menu_puton[i].sold_out,
                        item_id:menu_puton[i].item_id,
                        category_id:this.lot_manage_category_id,
                    discount: this.pickerDiscount / 100
                    };
                    menu_puton_list.push(data)
                }

                try{
                    const result = await updateDiscount(menu_puton_list);
                    if (result.status == 1) {
                        this.showAlert = true;
                        this.alertText = '割引率変更成功'
                        this.pickerDiscount = -1
                    }
                    else{
                        this.showAlert = true;
                        this.alertText = '割引率変更失敗'
                    }
                }catch(err){
                    console.log(err)
                }
        ...
        waitingPicker() {
            ...
            this.map = this.maps[parseInt(this.pickerDiscount / 10)];
            this.map2 = this.maps[this.pickerDiscount % 10];
        },
        confrimPicker() {
            this.pickerDiscount = this.map.alias * 10 + this.map2.alias;
            this.pickerShow = false;
        },
            closeTip(){
                this.showAlert = false;
                this.coverShow = false;
                if(this.alertText!='料理を選択してください' && this.alertText!='割引率設定しない'){
                    //this.$router.go(-1);
                    this.initData();
                }
            },
...
<style lang="scss" scoped>
    ...
    .coverparts{
        ...
        .pickerW {
            display: inline-block;
            @include wh(4.2rem, 5.5rem);
            padding-top: 0.7rem;
            padding-left: 0.2rem;
            p {
                padding-left: 0.9rem;
                @include sc(1rem, #a4a4a4);
            }
        }
            .sa-icons{
                text-align: center;
