  ...
  <div class="vue-datepicker-panels" v-show="dayPanelIsShow || monthPanelIsShow || weekPanelIsShow || hourPanelIsShow || minutePanelIsShow">
      ...
      <table class="vue-datepicker-tb2">
        ...
        <tbody>
          <tr v-for="(season, index) in monthArr" :key="index" track-by="$index">
            <td v-for="(m, index) in season" :key="index" track-by="$index" @click="choiceMonth(m.id, $event)" :class="classMonth(m.id)">
              <span>{{ m.name }}</span>
            </td>
          </tr>
        </tbody>
    ...
    <div class="vue-datepicker-panel" v-show="dayPanelIsShow">
      ...
      <table class="vue-datepicker-tb">
        <thead>
          <tr>
            <th v-for="(d, index) in langConf.week" :key="index">{{ d }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(m, index) in monthDays" :key="index" track-by="$index">
            <td v-for="(d, index) in m" track-by="$index" :key="index" @click="choiceDay(d, $event)" :class="classDay(d)">
              <span>{{ d }}</span>
            </td>
          </tr>
        </tbody>
...
function getWeekData(year){
  ...
  for(let i of list){
      ...
      let weekShortText = "第"+weekIndex+"週 "+weekStart;
      let weekText = "第"+weekIndex+"週 "+weekStart+" ~ "+weekEnd;
